...
if (isset($_POST['id'], $_POST['key_name'])) {
    ...
    $stmt = $conn->prepare("SELECT file_name FROM aboutus_archive WHERE id=? AND key_name=?");
    ...
    if ($restoreFile) {
        ...
        if (file_exists($archivePath)) {
            ...
            $stmt = $conn->prepare("DELETE FROM aboutus_archive WHERE id=?");
...
header("Location: ../../components/admin_dashboard/aboutus_archive.php");
exit;
